...
checkpoint_path = "model-checkpoint-1.pt"
...
for epoch in range(epochs_trained, total_epochs):
    ...
    for x, y in data_loader:
        ...
        with torch.no_grad():
            yp = model(data.xt)
            loss = criterion(yp, data.yt)
            losses.append(loss)

checkpoint = {"epochs_trained": total_epochs, "optimizer": optimizer.state_dict(),
              "model": model.state_dict()}

model_save_path = "model-checkpoint-2.pt"
